...
/* Parse an hourly limit  */
gint
twitux_parser_hourly_limit (const gchar *data,
						  gssize       length)
{
	xmlDoc		*doc = NULL;
	xmlNode		*root_element = NULL;
	...
	for (cur_node = root_element; cur_node; cur_node = cur_node->next) {
		if (cur_node->type != XML_ELEMENT_NODE)
			continue;
		if (g_str_equal (cur_node->name, "remaining-hits") || g_str_equal (cur_node->name, "reset-time-in-seconds")){
			const xmlChar *tmp;
			xmlBufferPtr	buffer;

			buffer = xmlBufferCreate ();
			if (xmlNodeBufGetContent (buffer, cur_node) != 0) {
				g_critical("Wasn't able to get xml buffer");
				continue;
			}

			tmp = xmlBufferContent(buffer);

			if (g_str_equal (cur_node->name, "remaining-hits"))
				limit = atoi((char*)tmp);
			else
				reset = atoi((char*)tmp);

			xmlBufferFree (buffer);
		} else if (g_str_equal (cur_node->name, "hash")){
			cur_node = cur_node->children;
		}
	} /* End of loop */
	...
	else /* limit may have been set to something, but we don't have the reset time, so drop it back to "don't know" */
		limit = -1;
